  ...
  for (const word of words.slice(1)) {
    ...
    if (speakerChanged || tooLong || sentenceEnded) {
      segments.push({
        ...
        speaker: previous.speaker == null ? null : `Speaker ${previous.speaker + 1}`,
...
async function transcribeDeepgram(audioPath: string): Promise<NormalizedTranscript> {
  ...
  const response = await fetch('https://api.deepgram.com/v1/listen?model=nova-3&smart_format=true&punctuate=true&paragraphs=true&utterances=true&diarize=true', {
    method: 'POST',
    headers: {
...
function fixtureTranscript(durationSeconds: number): NormalizedTranscript {
  ...
  const segmentLength = Math.max(5, Math.min(12, durationSeconds / text.length));
  const segments = text.map((value, index) => ({
    sequence: index,
    startSeconds: index * segmentLength,
    endSeconds: Math.min(durationSeconds, (index + 1) * segmentLength),
    text: value,
    confidence: 1,
